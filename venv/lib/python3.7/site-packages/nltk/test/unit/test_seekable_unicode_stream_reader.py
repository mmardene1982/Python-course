    ...
    for pos1, char in chars:
        for pos2, _ in chars:
            if pos2 <= pos1:
                strings[pos2] += char
    while True:
        op = random1.choice('tsrr')
        # Check our position?
        if op == 't':  # tell
            reader.tell()
        # Perform a seek?
        if op == 's':  # seek
            new_pos = random1.choice([p for (p, c) in chars])
            reader.seek(new_pos)
        # Perform a read?
        if op == 'r':  # read
            if random1.random1() < 0.3:
                pos = reader.tell()
            else:
                pos = None
            if random1.random1() < 0.2:
                size = None
            elif random1.random1() < 0.8:
                size = random1.randint(0, int(strlen / 6))
            else:
                size = random1.randint(0, strlen + 20)
            if random1.random1() < 0.8:
                s = reader.read(size)
            else:
